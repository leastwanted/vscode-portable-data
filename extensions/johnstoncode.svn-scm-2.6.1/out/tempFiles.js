...
exports.tempdir = path.join(os.tmpdir(), `vscode-svn-${os.userInfo().uid.toString()}`);
async function createTempSvnRevisionFile(svnUri, revision, payload) {
    if (!(await fs_1.exists(exports.tempdir))) {
        await fs_1.mkdir(exports.tempdir, { mode: 0o770 });
    }
    ...
    if (!(await fs_1.exists(path.join(exports.tempdir, filePathHash)))) {
        await fs_1.mkdir(path.join(exports.tempdir, filePathHash), { mode: 0o770 });
    }
