...
function exportDiagram(diagram, format, savePath, bar) {
    if (bar) {
        bar.show();
        bar.text = common_1.localize(7, null, diagram.name + "." + format.split(":")[0]);
    }
    let renderTask = appliedRender_1.appliedRender(diagram.parentUri).render(diagram, format, savePath);
    ...
    let mapTask = appliedRender_1.appliedRender(diagram.parentUri).getMapData(diagram, cmapx);
