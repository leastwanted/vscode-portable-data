...
class LocalRender {
    ...
    createTask(diagram, taskType, savePath, format) {
        ...
        let pms = [...Array(diagram.pageCount).keys()].reduce((pChain, index) => {
            ...
            let process = child_process.spawn(config_1.config.java(diagram.parentUri), params);
            ...
            return pChain.then(() => {
                ...
                let pms = processWrapper_1.processWrapper(process, savePath2).then(stdout => buffers.push(stdout), err => Promise.reject({ error: common_1.localize(10, null, diagram.name, err.error), out: err.out }));
                return pms;
            }, err => {
