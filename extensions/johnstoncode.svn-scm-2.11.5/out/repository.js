...
class Repository {
    ...
    async plainLogByRevision(revision) {
        return this.run(types_1.Operation.Log, () => this.repository.plainLogByRevision(revision));
    }
    async plainLogByText(search) {
        return this.run(types_1.Operation.Log, () => this.repository.plainLogByText(search));
    }
    ...
    async removeUnversioned() {
        return this.run(types_1.Operation.CleanUp, () => this.repository.removeUnversioned());
    }
    ...
    async loadStoredAuths() {
        ...
        try {
            return vscodeModules_1.keytar.findCredentials(this.getCredentialServiceName());
        }
        catch (error) {
            return [];
        }
    ...
    async saveAuth() {
        if (this.canSaveAuth && this.username && this.password) {
            try {
                await vscodeModules_1.keytar.setPassword(this.getCredentialServiceName(), this.username, this.password);
            }
            catch (error) {
                console.log(error);
            }
